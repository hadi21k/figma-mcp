...
export const NodeId = z.string().min(1).max(100).describe("Figma node ID, e.g. '123:456'");
...
export const StrokeConfig = z.object({
  color: RGBAColor,
...
export const LineHeight = z.union([
  z.object({ unit: z.literal("PIXELS"), value: z.number().positive() }).strict(),
  z.object({ unit: z.literal("PERCENT"), value: z.number().positive() }).strict(),
...
export const AutoLayoutAlign = z.enum(["MIN", "CENTER", "MAX", "SPACE_BETWEEN"]);
...
export const CreateFrameInput = z.object({
  name: z.string().min(1).max(500).describe("Frame name displayed in layers panel"),
...
export const SetAutoLayoutInput = z.object({
  ...
  gap: z.number().min(0).max(10000).default(0).describe("Spacing between children"),
  padding: Padding.default({ top: 0, right: 0, bottom: 0, left: 0 }),
  primaryAxisAlign: AutoLayoutAlign.default("MIN")
    .describe("Alignment along primary axis"),
  counterAxisAlign: z.enum(["MIN", "CENTER", "MAX"]).default("MIN")
...
export const CreateRectangleInput = z.object({
  ...
  fills: z.array(Fill).max(20).default([
...
export const AddShadowInput = z.object({
  nodeId: NodeId,
...
export const GroupNodesInput = z.object({
  nodeIds: z.array(NodeId).min(2).max(100).describe("At least 2 node IDs to group"),
...
export const ZoomToNodeInput = z.object({
  nodeId: NodeId,
}).strict();
...
export const TOOL_REGISTRY: Record<string, ToolDefinition> = {
  ...
  create_rectangle: {
    description: "Create a rectangle with optional rounded corners, fills, stroke",
  ...
  update_node: {
    description: "Update position, size, opacity, visibility, name, cornerRadius, or fills of any node",
